<template>
  <div class="mTree">
    <div class="tree_node">
      <img v-if="!isOpen && (isFolder || isDataSet)" src="../../assets/images/icon/add.png" class="triggle_open" @click="changeOpen" />
      <img v-if="isOpen && (isFolder || isDataSet)" src="../../assets/images/icon/plus.png" class="triggle_open" @click="changeOpen" />
      ...
      <img v-if="isOpen && isDataSet" src="../../assets/images/icon/dataSet.png" />
      <img v-if="!isOpen && isDataSet" src="../../assets/images/icon/dataSetNormal.png" />
    ...
    <div v-if="isOpen && isFolder && childrenNodes.length">
      ...
      >
        <mTree
          v-if="copyToDataSet || item.isDataset <= 0"
    ...
    <template v-if="isOpen && isFolder && childrenNodes.length == 0">
      <div style="text-align: center;color: #ccc;">无子目录</div>
    </template>
  </div>
</template>
<script>
...
export default {
  ...
  inject: ['handleNodeSelect', 'copyToDataSet'],
  computed: {
    isFolder() {
      return this.node.isDataset == '0';
    },
    isDataSet() {
      return this.node.isDataset > 0
    }
  ...
  methods: {
    ...
    handleNodeClick() {
      if(this.copyToDataSet && this.node.isDataset <= 0) {
        this.$Message.error('数据只能复制到数据集中!')
        return;
      }
      ...
      this.handleNodeSelect(this.node)
    }
  },
  mounted() {
  }
