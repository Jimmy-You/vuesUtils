...
<template>
	<div class="uploadWrapper" v-if="isLogin">
		<div v-show="isShow">
			<div class="title">
				<div>{{uploadStatus > 0 ? `正在上传${uploadStatus}个项目` : '上传列表'}}</div>
				<div class="ops">
					<Button size="small" type="primary" @click="handleClearSuccess" style="margin-right: 6px;">清除已完成</Button>
						...
						<div class="upload_item" v-for="(item, index) in uploadShowList" :key="index">
							...
							<div class="item_detail">
								...
								<div class="m_progress_title">
									...
									<span v-if="item.status == 1 && item.progress != 100">{{item.progress}}%</span>
		...
		<div
      v-show="!isShow && uploadShowList.length"
      class="upload-tag"
      :style="{animation: uploadShowList.length ? 'uploading 5s infinite' : ''}"
      @click="changeUploadShow"
    >
      <img src="@/assets/images/icon/uploadMenu.png" />
      <div>上传任务列表</div>
    </div>
	</div>	
</template>
	...
	changeUploadShow() {
		...
		getSpeed(item) {
			...
			if(item.progress && item.oldProgress) {
				...
				const speed = fileSize * ( Number((item.percentage * 100).toFixed(2)) - item.oldProgress) / 100 * 2
				return this._renderSize(speed)
			}
		},
		handleLoginLogingout() {
			this.isShow = false;
			this.isLogin = false;
			this.uploadShowList = [];
		},
		handleClearSuccess() {
			this.uploadShowList.forEach(item => {
				...
				if(item.status == 2) {
					delete this.$uploadUtil.uploadListObj[uuid]
				}
			})
			this.uploadShowList = Object.keys(this.$uploadUtil.uploadListObj).map((item) => {
				return this.$uploadUtil.uploadListObj[item]
			}) || []
		}
	},
}
